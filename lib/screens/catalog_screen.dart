...
class _CatalogScreenState extends State<CatalogScreen> {
  ...
  static const _categories = [
    {'label': 'Клавишные',   'icon': Icons.piano, 'image': 'assets/categories/piano.png'},
    {'label': 'Струнные',    'icon': Icons.queue_music, 'image': 'assets/categories/guitar.png'},
    {'label': 'Духовые',     'icon': Icons.music_note, 'image': 'assets/categories/saxophone.png'},
    {'label': 'Ударные',     'icon': Icons.speaker, 'image': 'assets/categories/drums.png'},
    {'label': 'Электронные', 'icon': Icons.electrical_services, 'image': 'assets/categories/synthesizer.png'},
    {'label': 'Аксессуары',  'icon': Icons.build_outlined, 'image': 'assets/categories/accessories.png'},
  ];
  ...
  void _onScroll() {
    ...
    if (px >= _scrollController.position.maxScrollExtent - 200) { _loadMore(); }
  }
  ...
  int get _activeFilterCount =>
      (_category != null ? 1 : 0) + (_tag != null ? 1 : 0) + (_highRating ? 1 : 0);
  ...
  List<Product> get _filtered {
    ...
    final list = all.where((p) {
      ...
      return matchCat && matchTag && matchRating && matchSearch && matchPrice;
    }).toList()
                  ...
                  child: Column(
                    children: [
                      ...
                      TextField(
                        ...
                        onChanged: (v) => setState(() { _search = v; _searchActive = v.isNotEmpty; _reset(); }),
                        onSubmitted: (v) {
                      ...
                      onTap: () {
                        ...
                        WidgetsBinding.instance.addPostFrameCallback((_) {
                          if (_scrollController.hasClients) {
                            _scrollController.jumpTo(currentOffset.clamp(
                              0.0,
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      {'label': 'Хит', 'icon': Icons.local_fire_department},
                      {'label': 'Новинка', 'icon': Icons.fiber_new},
                      {'label': 'Скидка', 'icon': Icons.sell_outlined},
                    ].map((item) {
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                            ...
                            _ActiveChip(
                              ...
                              onDeleted: () {
                                final currentOffset = _scrollController.hasClients 
                                    ? _scrollController.offset 
                                    : 0.0;
                                setState(() { _category = null; _reset(); });
                                WidgetsBinding.instance.addPostFrameCallback((_) {
                                  if (_scrollController.hasClients) {
                                    _scrollController.jumpTo(currentOffset.clamp(
                                      0.0,
                    ...
                    if (_category != null || _tag != null || _sort != _Sort.none)
                      TextButton(
                        onPressed: _resetAll,
                        style: TextButton.styleFrom(
                            foregroundColor: Colors.grey, padding: EdgeInsets.zero),
                        child: const Text('Сбросить всё', style: TextStyle(fontSize: 12)),
                      ),
                  ],
                ),
              ),
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => ProductDetailScreenEntry(product: product)),
      ),
      child: Card(
                        ...
                        Row(
                          children: [
                            ...
                            if (inCart)
                              Row(
                                children: [
                                  GestureDetector(
                                    ...
                                    child: Icon(Icons.remove_circle, color: scheme.primary, size: 22),
                                  ),
                                  Padding(
                                  ...
                                  GestureDetector(
                                    ...
                                    child: Icon(Icons.add_circle, color: scheme.primary, size: 22),
                                  ),
                                ],
                              )
                            else
                              GestureDetector(
                                onTap: () => context.read<CartProvider>().add(product),
                                child: Icon(Icons.add_circle, color: scheme.primary, size: 24),
                              ),
                          ],
                        ),
                      ],
